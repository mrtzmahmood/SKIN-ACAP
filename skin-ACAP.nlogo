...
;   initial-capital              ; the capital that a firm starts with
  ...
  initial-capital-for-research-laboratories  ; the amount of start-up capital for those firms set to be 'research laboratory'
...
firms-own [;modified
  ...
  last-innovation-time
  innovation-time-length
  research-type                   ;applied or basic research
  ...
  adverted-ih                     ; part of ih which is usable by other firms
  new-ih?                         ; true when a new IH has been generated
  advert                          ; a list of the capabilities of my innovation 
                                  ;     hypothesis  
  ;research
  firm-type                       ;firm (small or big) or research laboratory
  ...
  less-targeted-partners
  more-targeted-partners
...
to setup ;modified
  ...
  set initial-capital-for-research-laboratories initial-capital * 100
...
to initialise-firms ;modified
  ...
  ask n-of round ((big-firms-percent / 100) * nFirms) firms [ ;;bug fix for v 5-32: added /100
    ...
    set firm-type "big"
    set research-type "mixed" ;applied and basic research
  ]
  ask n-of round ((research-laboratories-percent / 100) * nFirms) firms [ 
    set capital initial-capital-for-research-laboratories 
    set color green
    set firm-type "research laboratory"
    set research-type "mixed"
    set capital-knowledge-ratio 1
  ]
...
to initialise-firm
  ...
  set less-targeted-partners no-turtles
  set more-targeted-partners no-turtles
  ...
  set partner-links []
  set total-links 0  
  set first-innovation-time 0
  set last-innovation-time iterations
  set innovation-time-length last-innovation-time - first-innovation-time
end

; set up a single firm with all its required knowledge

to setup-firm ;modified
  make-kene
  make-innovation-hypothesis
  ;if firm-type != "research laboratry" [make-advert]
  make-advert
  if firm-type != "research laboratry" [manufacture]
end
...
to go ;modified
  ...
  ask firms with [firm-type != "research laboratory"] [ 
    if partnership-strategy != "no-partnering" [ collaborate ]
    do-research 
    manufacture
    pay-taxes
  ]
  ...
  let industry-capital sum [capital] of firms with [firm-type != "reserach laboratories"]
  ask firms [
    let capital-share capital / industry-capital
    set capital2 capital-share ^ 2
    ]
  create-partners-list
  create-start-ups
  create-nets
  ...
  ask firms with [firm-type != "research laboratory"] [ do-admin ]
  results-update
  
  ask firms with [(firm-type != "research laboratory") and trading?] [
     if last-innovation-time != iterations
     [set first-innovation-time last-innovation-time]
     set last-innovation-time ticks
     set innovation-time-length last-innovation-time - first-innovation-time
  ]
  set-current-plot "speed-of-innovation"
  set-current-plot-pen "speed"
  let firm-count count firms with [firm-type != "research laboratory"]
  plot iterations - ((sum [innovation-time-length] of firms with [firm-type != "research laboratory"]) / firm-count)

  if ticks >= Iterations
 [  
;   make gdf file format
   file-open "C:\\Users\\mahmoudzadeh\\Desktop\\output\\g7-3.gdf"
   file-print "nodedef> name VARCHAR, label VARCHAR, tip VARCHAR"
   let ft "" 
   ask firms [
        ifelse [firm-type] of self = "small" 
        [set ft "s"]
        [ifelse [firm-type] of self ="big"
          [set ft "b"]
          [set ft "l"]
        ]
      file-print (word who ", " ft ", " ft)
   ]
   file-print "edgedef> node1, node2"
   let edge-nodes []
   ask links [
     set edge-nodes []
     ask both-ends [
     ...
     set edge-nodes lput who edge-nodes    
     ]
     file-print (word (item 0 edge-nodes) "," (item 1 edge-nodes))
   ]
   file-close   

;   file-open "C:\\Users\\mahmoudzadeh\\Desktop\\output\\statistics1.txt"
;   set ft "" 
;   ask firms [
;        ifelse [firm-type] of self = "small" 
;        [set ft "s"]
...
;          [set ft "l"]
;        ]
;      file-print (word ft who "," (length ih))
;   ]
;   file-close   

;     foreach firms-population [file-write ? 
;                               file-print " "]
;     file-close
;     file-open "C:\\Users\\mahmoudzadeh\\Desktop\\SDC-Paper\\s41\\ihlength.txt"
;     foreach mean-ih-length [file-write ? 
;                             file-print " "]
;     file-close
;     file-open "C:\\Users\\mahmoudzadeh\\Desktop\\SDC-Paper\\s41\\herfindahl.txt"
;     foreach h-index [file-write ? 
;                      file-print " "]
;     file-close
;     file-open "C:\\Users\\mahmoudzadeh\\Desktop\\SDC-Paper\\s41\\netdensity.txt"
;     foreach net-density [file-write ? 
;                          file-print " "]
;     file-close
     stop
  ]
...
to create-partners-list
   let p-links []
   ask firms with [(count my-links) != 0] [
      set total-links count my-links
      set p-links []
      ask my-links [set p-links lput other-end p-links]
      set partner-links sort p-links
  ]
end

to show-links
  no-display
    let supplier-set turtle-set (filter [is-firm? ?] suppliers)
    let customer-set turtle-set (filter [is-firm? ?] customers)
    ;create-links-with customer-set [set color gray]
    ifelse partnership-strategy = "absorptive capacity" 
    [
      create-links-with less-targeted-partners  [set color brown]  
      create-links-with more-targeted-partners  [set color red]
    ]
    [if partnership-strategy != "absorptive capacity" [create-links-with partners  [set color green]]]
end

to do-laboratory-research
    ifelse extra-industry-knowledge-production-rate = "high"
    [if (ticks mod 10) = 0 [ask firms with [firm-type = "research laboratory"] 
        [do-radical-research]]] ;produce completely new knowledge each 10 period
  [ifelse extra-industry-knowledge-production-rate = "medium"
    [if (ticks mod 20) = 0 [ask firms with [firm-type = "research laboratory"] 
        [do-radical-research]]] ;produce completely new knowledge each 20 period
    [if (ticks mod 50) = 0 [ask firms with [firm-type = "research laboratory"] 
        [do-radical-research]]] ;produce completely new knowledge each 50 period
   ]
end

to results-update
  set mean-ih-length lput precision (mean [length ih] of firms with [firm-type != "research laboratories"]) 2 mean-ih-length 
  set firms-population lput count firms with [firm-type != "research laboratories"] firms-population
  set h-index lput (100 * sum [capital2] of firms with [firm-type != "research laboratories"]) h-index
  set net-density lput ((sum [total-links] of firms with [firm-type != "research laboratories"]) / (2 ^ ((100 - research-laboratories-percent) * nFirms / 100 ))) net-density
end
...
;firm procedure
to make-advert;modified
  let location 0
  let change-advert? false
  let ih-length length ih
  let ih-medium-length (round (ih-length * 0.6)) + 1
  let ih-exclusive-length (round (ih-length * 0.2)) + 1
  if (partnership-strategy = "absorptive capacity") and (intra-industry-knowledge-spillover-status = "medium") 
    [; 60% of capabilities could spillover
          set adverted-ih []
          while [ ih-medium-length > 0 ] [
          set location random ih-length
           if not member? location adverted-ih [
              set adverted-ih fput location adverted-ih
              set ih-medium-length ih-medium-length - 1
           ]
        ]
        set adverted-ih sort adverted-ih
        set change-advert? true
     ] 
  if (partnership-strategy = "absorptive capacity") and (intra-industry-knowledge-spillover-status = "exclusive")       
    [ ; 20% of capabilities could spillover
          set adverted-ih []
          while [ ih-exclusive-length > 0 ] [
             set location random ih-length
             if not member? location adverted-ih [
                set adverted-ih fput location adverted-ih
                set ih-exclusive-length ih-exclusive-length - 1
             ]
          ]
          set adverted-ih sort adverted-ih
          set change-advert? true
      ]
    if not change-advert? [ set adverted-ih ih]
    set advert map [ item ? capabilities ] adverted-ih  
end
...
; observer procedure
to find-suppliers;modified
  ask firms with [firm-type != "research laboratory"]  [set selling? false ]
  let possible-firms firms with [ inputs-available firms with [firm-type != "research laboratory"] ]
  ...
  ask firms with [firm-type != "research laboratory"] [set trading? false ]
...
; observer procedure
to buy;modified
  ask firms with [firm-type != "research laboratory"] [ set sales 0 set customers [] ]
  ask firms with [(firm-type != "research laboratory") and (trading? = true) ] [
    purchase
  ]
end
...
;firm procedure
to collaborate
  if Partnership-strategy = "no-partnering" [ stop ]
  if not trading? [
    ifelse (partnership-strategy = "absorptive capacity") [
       find-less-targeted-partners
       find-more-targeted-partners
       if any? more-targeted-partners  [ learn-from-partners ]
       if any? less-targeted-partners  [ learn-from-partners ]
       pay-tax collaboration-tax * count more-targeted-partners
    ]
    [find-partners
...
to find-less-targeted-partners
  let my-candidates no-turtles
  let research-candidates no-turtles
  let other-firms-candidates no-turtles
  let candidates no-turtles

  set my-candidates (turtle-set previous-partners (filter [is-firm? ?] suppliers) (filter [is-firm? ?] customers))
  set research-candidates firms with [firm-type = "research laboratories"]
  set other-firms-candidates firms with [firm-type != "research laboratories"]
  
  set other-firms-candidates other-firms-candidates with [ (not member? self my-candidates) and self != myself]
    
  set candidates (turtle-set research-candidates other-firms-candidates)
  set candidates candidates with [less-compatible? myself ]
  set less-targeted-partners (turtle-set less-targeted-partners candidates)
  ; add the new partner to my partners 
  ask candidates [ set less-targeted-partners (turtle-set myself less-targeted-partners) ] 
end 

to find-more-targeted-partners
  let candidates no-turtles
  
  set candidates (turtle-set previous-partners (filter [is-firm? ?] suppliers) (filter [is-firm? ?] customers))
  set candidates candidates with [more-compatible? myself ]
  set more-targeted-partners (turtle-set more-targeted-partners candidates)
  ask candidates [ set more-targeted-partners (turtle-set myself more-targeted-partners) ] 
...
to-report less-compatible? [ possible-partner ]
  ...
  if net != nobody and member? possible-partner [members] of net [ report false ]
  ; a possible partner cannot already be a partner of mine
  if member? self [less-targeted-partners] of possible-partner or member? possible-partner less-targeted-partners [report false]    
  set attractiveness (length intersection advert [advert] of possible-partner) /
      (min list length advert length [advert] of possible-partner)
...
to-report more-compatible? [ possible-partner ]
...
to add-capabilities [ other-firm ]
  let my-position 0
  foreach [adverted-ih] of other-firm [
...
to do-radical-research
  if (firm-type != "research laboratories") and (not Rad-research) [ stop ]
...
to do-admin            
  ...
  ifelse (partnership-strategy != "absorptive capacity") and (partnership-strategy != "no-partnering")
  [set previous-partners (turtle-set previous-partners partners)]
  [if partnership-strategy = "absorptive capacity" [set previous-partners (turtle-set previous-partners more-targeted-partners)]]
  set age age + 1 
  ; die if bankrupt
  if capital < 0 [ exit ]
end
...
to make-start-up
  create-firms 1 [
    ...
    setxy random-xcor random-ycor
    initialise-firm
    let firm-2-clone max-one-of other firms [ last-reward ]
    clone-kene firm-2-clone
    set firm-type [firm-type] of firm-2-clone
    make-innovation-hypothesis
    make-advert
    manufacture
  ] 
...
to show-plots
    ...
    set-current-plot "average ih length"
    set-current-plot-pen "Average"
    plot mean [length ih] of firms 
    
    set-current-plot "network density"
    set-current-plot-pen "total"
    let n count firms with [firm-type != "research laboratories"]
    plot 100 * ((sum [total-links] of firms) / (n * (n - 1) / 2))

    set-current-plot "Herfindahl index"
    set-current-plot-pen "total"
    plot 100 * sum [capital2] of firms
...
;    plot count networks     
    ...
    set-current-plot-pen "selling"
    plot 100 * count firms with [ selling? ] / count-of-firms
    set-current-plot-pen "trading"
...
partnership-strategy
"no-partnering" "conservative" "progressive" "absorptive capacity"
3
...
"" ""
PENS
"selling" 1.0 0 -7500403 true "" ""
"trading" 1.0 0 -6459832 true "" ""
...
550
60
SKIN (Simulating Knowledge Dynamics in Innovation Networks) 5.31\nExtended by Adding Absorptive Capacity and Knowledge Spill over Concepts
11
0.0
1
...
495
460
extra-industry-knowledge-production-rate
extra-industry-knowledge-production-rate
"high" "medium" "low"
0
...
495
510
intra-industry-knowledge-spillover-status
intra-industry-knowledge-spillover-status
"exclusive" "medium" "public"
2
